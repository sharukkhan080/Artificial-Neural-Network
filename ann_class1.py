...
class ANN(object):
    ...
    def plot_weights_over_epochs(self):
        for i in range(0, len(self.parameters)//2):
            ...
            plt.xlabel("Epochs")
            plt.ylabel("Weights")
            plt.plot(self.weights["W" + str(i+1)],'r')
            #plt.boxplot(self.weights["W" + str(i+1)],vert=True,patch_artist=True)
            #sns.kdeplot(self.weights["W" + str(i+1)])
            #sns.rugplot(self.weights["W" + str(i+1)])
            #sns.distplot(self.weights["W" + str(i+1)],kde=False,bins=30)
            #sns.countplot(self.weights["W" + str(i+1)],data=data)
            #sns.jointplot(self.weights["W" + str(i+1)],kind='scatter')
            plt.show()
            
            '''fig, axes = plt.subplots()
            axes.plot(self.weights["W" + str(i+1)])
            axes.set_xlabel('Epochs')
            axes.set_ylabel('Weights')
            axes.set_title("W" + str(i+1) +" over epochs");
            plt.show()'''
